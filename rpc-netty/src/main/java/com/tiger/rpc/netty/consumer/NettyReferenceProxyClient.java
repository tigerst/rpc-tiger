...
/**
 * @ClassName: NettyReferenceProxyClient.java
 *
...
@Slf4j
public class NettyReferenceProxyClient {

	/**
	 * 引入发现服务
	 */
	private NettyServiceDiscovery discovery;
...
//		ClassLoader classLoader = Thread.currentThread().getContextClassLoader();
	...
	public <T> T getDirectorProxy(Class<T> iFaceInterface, List<String> hostPorts) throws ServiceException {
		...
		//小集群地址解析 & 检测
		List<String> uris = UriUtils.getUris(hostPorts, ProtocolTypeEnum.NETTY.getValue());
		...
		//初始化基于服务发现器的直连代理
		NettyDirectorHandler handler = new NettyDirectorHandler(discovery);
		//设置连接池
		handler.setPool(this.discovery.getPool());
		handler.setServiceVersion(config.getVersion());
		if(config.getRetry() >= 0){
...
//		ClassLoader classLoader = Thread.currentThread().getContextClassLoader();
	 ...
	 * @param discovery
	 */
	private void checkDiscovery(NettyServiceDiscovery discovery) {
		//校验代理
